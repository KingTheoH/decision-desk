...
export default function ContentIdeas() {
  ...
  const load = () => decisions.list({ type: "ContentIdea" }).then(setItems);
  useEffect(() => { load(); }, []);
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold text-zinc-100">Content Ideas</h1>
        <button className="btn btn-primary" onClick={() => setShowNew(true)}><Plus size={13} /> New</button>
      </div>
      {items.length === 0 ? (
        <div className="card p-8 text-center text-zinc-600 text-sm">No content ideas yet.</div>
      ) : (
